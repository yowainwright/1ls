...
test("chunk splits array into chunks", () => {
  ...
  expect(executeBuiltin("chunk", [1, 2, 3, 4, 5, 6], [3])).toEqual([
    [1, 2, 3],
    [4, 5, 6],
  ]);
...
test("compact removes falsy values", () => {
  expect(executeBuiltin("compact", [0, 1, false, 2, "", 3, null, undefined], [])).toEqual([
    1, 2, 3,
  ]);
...
test("toPairs converts object to array of pairs", () => {
  expect(executeBuiltin("toPairs", { a: 1, b: 2 }, [])).toEqual([
    ["a", 1],
    ["b", 2],
  ]);
