...
export const processDirectoryEntry = async (
  ...
  const childFiles = shouldRecurse ? await walkDirectory(fullPath, depth + 1, options) : [];
...
export const walkDirectory = async (
  ...
  const fileArrays = await Promise.all(
    entries.map((entry) => processDirectoryEntry(currentDir, entry, depth, options)),
  );

  return fileArrays.flat();
};

export const listFiles = async (dir: string, options: ListOptions = {}): Promise<FileInfo[]> =>
