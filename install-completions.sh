...
if [ "$SHELL_NAME" = "bash" ]; then
    ...
    if [ -d "$HOME/.bash_completion.d" ]; then
        cp "$SCRIPT_DIR/completions/1ls.bash" "$HOME/.bash_completion.d/"
        ...
        echo "source $SCRIPT_DIR/completions/1ls.bash" >> "$HOME/.bashrc"
        echo "Added to ~/.bashrc"
    else
        echo "Could not find suitable location for bash completions"
        echo "Please manually source: $SCRIPT_DIR/completions/1ls.bash"
    fi
elif [ "$SHELL_NAME" = "zsh" ]; then
    # Zsh installation
    if [ -d "$HOME/.zsh/completions" ]; then
        cp "$SCRIPT_DIR/completions/1ls.zsh" "$HOME/.zsh/completions/_1ls"
        echo "Installed to ~/.zsh/completions/"
    elif [ -d "/usr/local/share/zsh/site-functions" ]; then
        cp "$SCRIPT_DIR/completions/1ls.zsh" "/usr/local/share/zsh/site-functions/_1ls"
