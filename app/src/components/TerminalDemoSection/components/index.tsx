...
function TerminalCardContent({ files }: TerminalCardContentProps) {
  return (
    <div className="relative bg-[#1e1e1e]">
      <Sandpack template="static" files={files} options={SANDPACK_OPTIONS} theme="dark" />
    </div>
  );
}
  ...
  return (
    <div className="space-y-2 border-t border-border/10 bg-muted/30 px-4 py-3">
      <div>
        ...
        <code className="mt-1 block font-mono text-xs text-foreground">{command}</code>
      </div>
      {output && (
        <div>
          <div className="text-xs font-medium text-muted-foreground">Output:</div>
          <code className="mt-1 block font-mono text-xs text-primary">{output}</code>
        </div>
      )}
    </div>
  );
