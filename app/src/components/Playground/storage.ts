...
export const saveState = (state: Omit<StoredState, "savedAt">): Effect.Effect<void> =>
  Effect.gen(function* () {
...
export const loadState = (): Effect.Effect<StoredState | null> =>
  Effect.gen(function* () {
    const db = yield* openDB;
    ...
    const result = yield* Effect.tryPromise({
      try: () =>
        new Promise<StoredState | null>((resolve, reject) => {
